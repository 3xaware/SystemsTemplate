...
## Componentes
- **SceneManager.gd** (`Node`): nodo orquestador que instancia nuevas escenas, destruye la anterior y emite `EventBus.scene_loaded(scene: Node)` cuando finaliza el cambio.
...
1. **Añadir SceneManager**
   ...
   - Conecta la señal global `EventBus.scene_loaded` si necesitas ejecutar lógica justo después de que una escena termine de instanciarse.
...
4. Se instancia la nueva escena, se añade al árbol raíz y se emite la señal global `EventBus.scene_loaded`.
...
## Components
- **SceneManager.gd** (`Node`): orchestrator node that instantiates new scenes, frees the previous one, and emits `EventBus.scene_loaded(scene: Node)` when the swap finishes.
...
1. **Add SceneManager**
   ...
   - Connect the global `EventBus.scene_loaded` signal if you need to run logic right after a scene finishes instantiating.
...
4. The new scene is instantiated, added to the root, and the global `EventBus.scene_loaded` signal is emitted.
